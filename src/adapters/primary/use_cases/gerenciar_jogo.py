...
def iniciar_jogo(tamanho_tela_largura, tamanho_tela_altura):
    ...
    while True:
        if (gerenciar_menus.rodar_menu_inicial(menu_inicial)):
            ...
            executar_jogo(jogo_model, jogo_ui, fase_ui, menu_fase, item_service, personagem_service, menu_inicial, menu_perdeu, tamanho_tela_largura, tamanho_tela_altura)
            break

def executar_jogo(jogo_model, jogo_ui, fase_ui, menu_fase, item_service, personagem_service, menu_inicial, menu_perdeu, 
                  tamanho_tela_largura, tamanho_tela_altura):
    while True:
            gerenciar_fase.setar_fase(jogo_model, 1)
            gerenciar_fase.iniciar_fase(menu_fase, jogo_model, jogo_ui, fase_ui, 
                                        item_service, personagem_service, menu_inicial)
                    
            if (jogo_model.fase_perdida):
                print("Entrei aqui")
                if (gerenciar_menus.rodar_menu_perdeu(menu_perdeu)):
